function [Z,min_w,number_die] = subproblem_Z_2_1_new(N,lamda,earning_iteration_Zi,A)
percent=0.9;

number_die=0;
[movie_number,use_number] = size(A);
% 初始化 Z1 到 ZN 的值为 U
Z=zeros(1,movie_number);
Z(1,1:N) = use_number;  %初始化Z
    % 从第 N+1 件商品开始，逐步优化 Z
L_z = length(Z);  
%% 初始目标函数
min_w = 0;
